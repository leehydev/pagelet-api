...
export class NaverOAuthService {
  ...
  constructor(private readonly naverOAuthClient: NaverOAuthClient) {}
  ...
  async authenticate(code: string, redirectUri?: string): Promise<OAuthUserInfo> {
    try {
      this.logger.log(`Starting Naver OAuth authentication with code: ${code.substring(0, 10)}...`);

      // 1. Authorization Code를 Access Token으로 교환
      const tokenResponse = await this.naverOAuthClient.exchangeCodeForToken(code, redirectUri);
      this.logger.debug('Token exchange successful');

      // 2. Access Token으로 사용자 정보 조회
      const userInfo = await this.naverOAuthClient.getUserInfo(tokenResponse.access_token);
      ...
      this.logger.error(`Naver OAuth authentication failed: ${errorMessage}`);
